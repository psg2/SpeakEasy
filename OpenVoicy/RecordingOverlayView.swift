...
struct RecordingOverlayView: View {
    ...
    func barView(index i: Int, time: TimeInterval) -> some View {
        ...
        if isProcessing {
            ...
            let centerDist = abs(Double(i) - Double(numBars - 1) / 2.0) / (Double(numBars - 1) / 2.0)
            let centerFactor = 1.0 - centerDist * 0.5
            let variation = barOffsets[i] * 0.4 + 0.6
            let wave = sin(time * 5 + Double(i) * 0.7) * 0.1
            ...
            height = minHeight +
             ...
             let offset = sin(time * 3 + Double(i) * 0.3) * 1.0
